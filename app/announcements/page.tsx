  ...
  return (
    <div className="mx-auto max-w-2xl px-3 pb-32 pt-5 sm:px-6 lg:pb-16">
      <div className="mb-5 flex items-center gap-3">
        ...
        <div><h1 className="text-2xl font-semibold">Announcements</h1><p className="text-sm text-muted-foreground">News from the parish.</p></div>
      </div>
      {q.isLoading ? <FeedSkeleton /> : (
        <div className="space-y-4">
          {(q.data ?? []).map((a) => <AnnouncementPost key={a.id} item={a} />)}
          {q.data?.length === 0 && <p className="text-center text-sm text-muted-foreground">No announcements yet.</p>}
        </div>
      )}
    </div>
  );
