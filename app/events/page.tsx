  ...
  return (
    <div className="mx-auto max-w-2xl px-3 pb-32 pt-5 sm:px-6 lg:pb-16">
      <div className="mb-5 flex items-center gap-3">
        ...
        <div><h1 className="text-2xl font-semibold">Events</h1><p className="text-sm text-muted-foreground">What's happening at St. Catherine.</p></div>
      </div>
      {q.isLoading ? (
        ...
        <div className="space-y-3">
          {(q.data ?? []).map((e) => (
            <article key={e.id} className="card-surface card-surface-interactive overflow-hidden">
              ...
              <div className="flex items-center gap-4 p-4">
                <div className="num grid h-16 w-16 flex-none place-items-center bg-primary-soft text-center leading-none text-primary-deep">
          ...
          {q.data?.length === 0 && <p className="text-center text-sm text-muted-foreground">No events scheduled.</p>}
        </div>
      )}
    </div>
  );
